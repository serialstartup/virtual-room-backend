...
export class TryOnController {
    ...
    static async getTryOnById(req, res) {
        try {
            const { id } = req.params;
            const userId = req.user.userId;
    ...
    static async createTryOn(req, res) {
        try {
            const { self_image, model_image, avatar_id, dress_description, dress_image, product_url, } = req.body;
            const userId = req.user.userId;
            ...
            if (!dress_description && !dress_image && !product_url) {
                res.status(400).json({
                    success: false,
                    error: "At least one dress source (dress_description, dress_image, or product_url) is required",
                });
                return;
            }
            ...
            if (modelImageUrl && garmentImageUrl) {
                ...
                FashionAIService.processTryOnRequest(userId, tryOn.id, modelImageUrl, garmentImageUrl).catch((error) => {
                ...
                FashionAIService.processTextToImageTryOn(userId, tryOn.id, modelImageUrl, dress_description).catch((error) => {
    ...
    static async deleteTryOn(req, res) {
        try {
            const { id } = req.params;
            const userId = req.user.userId;
    ...
    static async classicTryOn(req, res) {
        try {
            const { self_image, model_image, avatar_id, dress_description, dress_image, product_url, } = req.body;
            const userId = req.user.userId;
    ...
    static async productToModel(req, res) {
        try {
            const { product_image, scene_prompt, product_name } = req.body;
            const userId = req.user.userId;
            if (!product_image) {
                res.status(400).json({
                    success: false,
            ...
            FashionAIService.processProductToModelTryOn(userId, tryOn.id, product_image, scene_prompt).catch((error) => {
    ...
    static async textToFashion(req, res) {
        try {
            const { fashion_description, scene_prompt } = req.body;
            const userId = req.user.userId;
            ...
            // Start Fashion AI processing
            FashionAIService.processCustomModelCreation(userId, customModel.id, fullPrompt, 'model-create').catch((error) => {
    ...
    static async avatarTryOn(req, res) {
        try {
            const { avatar_id, garment_image_url, garment_description, try_on_type = 'classic' } = req.body;
            const userId = req.user.userId;
            if (!avatar_id) {
                res.status(400).json({
                    success: false,
            ...
            if (try_on_type === 'classic' && garment_image_url) {
                FashionAIService.processTryOnRequest(userId, tryOn.id, avatar.avatar_image_url, garment_image_url).catch((error) => {
                    console.error("Avatar classic try-on processing failed:", error);
                });
            }
            else if (try_on_type === 'text-to-fashion' && garment_description) {
                FashionAIService.processTextToImageTryOn(userId, tryOn.id, avatar.avatar_image_url, garment_description).catch((error) => {
