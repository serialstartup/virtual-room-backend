...
export class TokenService {
   ...
   */
  static async hasSufficientTokens(userId: string, requiredAmount: number): Promise<boolean> {
    try {
   ...
   */
  static async useTokens(userId: string, usage: TokenUsageRequest): Promise<TokenTransaction> {
    try {
      // Start transaction
  ...
  static async creditTokens(
    ...
    amount: number, 
    description: string,
    transactionType: 'credit' | 'purchase' | 'gift' | 'refund' = 'credit'
   ...
   */
  static async giveWelcomeTokens(userId: string): Promise<TokenTransaction> {
    return this.creditTokens(
      userId,
      this.FREE_TOKENS_ON_SIGNUP,
      "Welcome gift - 5 free tokens",
      "gift"
    );
  }
   ...
   */
  static async refundTokens(userId: string, amount: number, reason: string, originalTryOnId?: string): Promise<TokenTransaction> {
   ...
   */
  static async validateAndUseTokensForTryOn(
    userId: string, 
    operationType: keyof typeof TokenService.TOKEN_COSTS,
    tryOnId: string
  ): Promise<{ success: boolean; transaction?: TokenTransaction; error?: string }> {
    try {
