...
export class StorageService {
  ...
  static async uploadFromUrl(
    imageUrl: string,
    folder: string = StorageService.FOLDERS.TRY_ONS,
    filename?: string
  ): Promise<string | null> {
    try {
      console.log(
        `[StorageService] 📥 Downloading image from: ${imageUrl.substring(
          0,
          50
        )}...`
      );
      ...
      console.log(
        `[StorageService] 📤 Uploading to Supabase: ${this.BUCKET_NAME}/${filePath}`
      );
      ...
      console.log(
        `[StorageService] ✅ Upload successful: ${publicUrlData.publicUrl}`
      );
