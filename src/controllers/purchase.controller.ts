...
import type { AuthenticatedRequest, TokenTransaction } from "../types/auth.js";
    ...
    try {
      ...
      if (transactionId) {
        ...
        if (existingTransaction) {
          console.log(
            `[PURCHASE] ⚠️ Transaction ${transactionId} already processed`
          );
          res.json({
            success: true,
      ...
      if (!purchases || purchases.length === 0) {
        console.error(
          `[PURCHASE] ❌ No purchases found for product ${productId}`
        );
        res.status(400).json({
          success: false,
      ...
      if (!validPurchase) {
        console.error(
          `[PURCHASE] ❌ Matching purchase not found for transaction ${transactionId}`
        );
        res.status(400).json({
          success: false,
      ...
      // 4. Credit tokens
      const description = `Purchase: ${productId} (RC: ${
