...
import { APIResponse, CreateUserAvatarRequest } from "../types/index.js";
import type { AuthenticatedRequest } from "../types/auth.js";

export class AvatarController {
  static async createAvatar(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const {
        name,
        face_image_url
      } = req.body as CreateUserAvatarRequest;
      ...
      FashionAIService.processAvatarCreation(
        userId,
        avatar.id,
        face_image_url
      ).catch((error) => {
  ...
  static async getAvatars(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const userId = req.user!.userId;
      ...
      const response: APIResponse<{avatars: typeof avatars}> = {
        success: true,
        data: { avatars },
      };
  ...
  static async getAvatarById(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const userId = req.user!.userId;
  ...
  static async updateAvatar(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const response: APIResponse<typeof updatedAvatar> = {
        success: true,
        data: updatedAvatar,
      };
  ...
  static async getAvatarStatus(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const userId = req.user!.userId;
      ...
      const response: APIResponse<{ status: string; avatar_image_url?: string }> = {
        success: true,
        data: { 
  ...
  static async retryAvatar(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const userId = req.user!.userId;
  ...
  static async getPrimaryAvatar(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const userId = req.user!.userId;
  ...
  static async setPrimaryAvatar(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const userId = req.user!.userId;
