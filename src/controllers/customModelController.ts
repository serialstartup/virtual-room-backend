...
import { APIResponse, CreateUserModelRequest } from "../types/index.js";
...
export class CustomModelController {
  static async createModel(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const {
        ...
        model_type,
        product_image
      } = req.body as CreateUserModelRequest;
      ...
      const response: APIResponse<typeof userModel> = {
        success: true,
        data: userModel,
      };
  ...
  static async updateModel(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const response: APIResponse<typeof updatedModel> = {
        success: true,
        data: updatedModel,
      };
  ...
  static async getModelStatus(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const userId = req.user!.userId;
      ...
      const response: APIResponse<{ status: string; image_url?: string }> = {
        success: true,
        data: { 
          status: model.status,
  ...
  static async retryModel(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const userId = req.user!.userId;
